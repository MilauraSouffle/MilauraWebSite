...
const CART_STORAGE_KEY = 'milaura-cart';
...
export const CartProvider = ({ children }) => {
    ...
    return new Promise((resolve, reject) => {
      if (variant.manage_inventory) {
        ...
        if ((currentCartQuantity + quantity) > availableQuantity) {
          const error = new Error(`Stock insuffisant pour ${product.title} (${variant.title}). Seulement ${availableQuantity} restant(s).`);
          reject(error);
          return;
        }
      }
  ...
  const getCartTotal = useCallback(() => {
    if (cartItems.length === 0) return formatCurrency(0, { code: 'EUR', symbol: '€' });
